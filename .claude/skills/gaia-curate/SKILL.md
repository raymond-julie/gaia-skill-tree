...
3. Normalize one candidate, exact-dedupe by canonical URL/content hash, then query `gaia dev list --generic --json`. Preserve its complete JSON array and canonical SHA-256 in the generic snapshot receipt; every mapping option must appear in that receipt. Offer at most three mapping options plus the bounded `NEW_GENERIC` and `UNSURE` paths; `UNSURE` is recorded as `DEFER`.
