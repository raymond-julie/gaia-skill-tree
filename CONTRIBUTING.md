...
- **Upstream Watcher (V1 — design ratified, implementation phased):** A background agent that polls the source repositories of curated **suites** for new releases and opens **GitHub issues** (not PRs, not branches) proposing version bumps, component adds/removes, and link liveness reports. Approved umbrella issues fire a small workflow that opens a draft `review/meta/` PR. Design: [`docs/agents/upstream-watcher.md`](docs/agents/upstream-watcher.md). Reviewer protocol for an `[upstream:release]` umbrella: read the machine-readable payload block, ensure all `upstream:child` intake issues are resolved (accept or reject each on standard intake criteria), then apply `upstream:approved` to the umbrella. Use `skip-child-gate` only when consciously accepting a version bump ahead of full component coverage.
