...
### Branch Workflow
- When starting fresh and indicating a PR, you are authorized to work on the PR branch right away. GO TO THE PR BRANCH, not the `claude/` or `gemini/` branches.

### Skills Intake
- When referring to skills that are not installed, check first if they are in the `.agents/` or `.claude/` directory.

### Token Spend Logging (Critical)
- On EVERY GitHub Project, LOG how much input / output token spend you made on which model + date.
  - *Example format*: `<date> Gemini Flash 3.5 High: 100k in, 200k out. ~$10`
