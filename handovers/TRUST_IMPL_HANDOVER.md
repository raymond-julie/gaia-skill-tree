...
| PR | Scope | Status |
|---|---|---|
| PR-1 | Schema (`grade`/`type`/`trustNumber`, meta thresholds + gate) | merged to `main` |
| PR-2 | Grading pipeline CLI (`--type`/`--trust`, `grading.py`, catalog injection) | merged to `main` (#686) |
| PR-2.5 | `gaia dev evidence --index` in-place re-grade + `evidence_graded` enum + bundled-schema sync (prerequisite gap fix discovered during PR-3) | PR #687 (draft) |
| PR-3 | Backfill — 220 generic-node entries re-graded | PR #688 (draft, stacked on #687) |
| Follow-up | Grade layering & inheritance for named skills + suite ultimates | issue #689 + `handovers/GRADE_LAYERING_HANDOVER.md` |
| PR-4 | Actionable reports (#648) | not started |

> **Note:** `handovers/TRUST_MODEL_RFC.md` v2 is referenced above but its full text lives on issue #646 (comment 4669151912); only this implementation handover and the per-PR briefs are committed here.
